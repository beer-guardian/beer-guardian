(function() {
    ...
    $(function() {//doc ready
      ...
      if (window.location.pathname == '/') {
        ...
        //figure this out please MORON
        $("#pass + .close").on('click', function() {
          $("#pass").val('');
        });

        //color the negative scores in red - rename to 'setScoreColor'
        scoreColor()

        //init tilt js cards with options if not safari. change to just chrome
        if (!isSafari()) {
        ...
        //main search bar enter key action
        ...
        //voting button clicks in modal and cards
    ...
    //open the main modal to show beer meta data
    //needs to get data from card that is clicked
    ...
    //open the secondary modal for searching brewerydb api
    function openAddBeerModal() {
      $('.modal-cover').fadeIn('fast')
      $('.add-beer-modal').addClass('reveal');
    }

    //close any modal and empty the search results
    function closeModal() {
    ...
    //get data from the brewerydb api and populate search ul
    ...
    //make a request to add the beer to the backend
    ...
    //vote up or down for a beer by id
    ...
    //refresh the score span on the cards when vote() has taken place
    ...
    //add a click event to dynamically added search results
    ...
    function consolePrint() {
      console.log("%c 👨🏻‍💻👨🏻‍💻🍺","font-size:90px")
      console.log("%c you're a curious little bugger aren't you", "background: #bababa; color: red;");
      ...
      console.log("%cGRIZZHACKS 2017 %s", css, 'Beer Guardian');
    }
